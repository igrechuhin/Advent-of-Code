...
private func getDay25Input() -> String {
    if testInput {
"""
jqt: rhn xhk nvd
rsh: frs pzl lsr
...
private typealias Graph = [String: Edge]
private typealias EdgesFrequency = [Edge: Int]
...
private extension Graph {
    func edgesFrequency() -> EdgesFrequency {

        var edgeFrequency: EdgesFrequency = [:]
...
//                    logger.debug("\(startIndex) : \(endIndex)")
                    ...
                    while !queue.isEmpty {
                        ...
                        self[current.name, default: []]
                            ...
                            .forEach { node in
                                ...
                                queue.append(
                                    Step(name: node, edges: current.edges + [[current.name, node]])
                                )
                            }
                    }
                }
    ...
    func removingEdges(_ edges: [Edge]) -> Graph {
        var graph = self
        for edge in edges {
            let pair = Array(edge)
...
public func solveDay25Puzzle1() -> Int {
