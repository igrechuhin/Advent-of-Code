...
extension AoC2024 {
  ...
  private static func getPuzzleInput() -> ([Int], [Int]) {
    ...
    var leftList = Array(repeating: 0, count: inputs.count)
    var rightList = Array(repeating: 0, count: inputs.count)
    
    for (index, input) in inputs.enumerated() {
      leftList[index] = input[0]
      rightList[index] = input[1]
    }
  ...
  static func solveDay1Puzzle2() -> Int {
    ...
    for item in rightList {
      countRight[item, default: 0] += 1
    }

    return leftList.reduce(0) {
      $0 + ($1 * countRight[$1, default: 0])
    }
  }
}
