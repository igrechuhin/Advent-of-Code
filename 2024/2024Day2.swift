...
private extension [Int] {
    ...
    var isReportSafe: Bool {
        safetyBreakIndex == nil
    }
    
    var isReportSafeForProblemDampener: Bool {
        reduced.isReportSafe || Array(reversed()).reduced.isReportSafe
    }
    
    var reduced: [Int] {
        guard let dropIndex = safetyBreakIndex else { return self }
        return self
            .enumerated()
            .filter { $0.offset != dropIndex }
            .map(\.element)
    }
}
