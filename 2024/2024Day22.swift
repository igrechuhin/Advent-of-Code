...
extension AoC2024 {
    ...
    static func solveDay22Puzzle1(_ mode: AoCMode) -> Int {
        getPuzzleInput(mode: mode)
            .components(separatedBy: .newlines)
            .filter(\.isNotEmpty)
            .compactMap { Int($0) }
            .map { secret in
                (1 ... 2000).reduce(secret) { secret, _ in
                    secret.nextSecret
                }
            }.sum
    }
    ...
    static func solveDay22Puzzle2(_ mode: AoCMode) -> Int {
        let inputsPrices = getPuzzleInput(mode: mode)
            .components(separatedBy: .newlines)
            .filter(\.isNotEmpty)
            .compactMap { Int($0) }
            .map(\.prices)
        ...
        return inputsPrices
            .parallelCompactMap { prices in
                (1 ... 9).reduce(Set<[Int]>()) { $0.union(prices.priceChanges(forPrice: $1)) }
            }
            .reduce(Set<[Int]>()) { $0.union($1) }
            .parallelCompactMap { changes in
                inputsPrices.map { $0.bananasCount(changes) }.sum
            }
            .max()!
    }
}
...
private extension [Int] {
    @inline(__always)
    func priceChanges(forPrice price: Int) -> Set<[Int]> {
        var result = Set<[Int]>()
        for index in indices where self[index] == price {
            if let priceChanges = priceChanges(atIndex: index) {
                result.insert(Array(priceChanges))
            }
        }
        return result
    }
    ...
    @inline(__always)
    func firstIndexMatchingChanges(_ changes: [Int]) -> Int? {
        let changes = Deque(changes)
        var indexChanges = Deque(priceChanges(atIndex: 4)!)
        if changes == indexChanges { return 4 }
        for i in 5 ..< count {
            _ = indexChanges.popFirst()
            indexChanges.append(self[i] - self[i - 1])
            if changes == indexChanges { return i }
        }
        return nil
    }
    
    @inline(__always)
    func bananasCount(_ changes: [Int]) -> Int {
        guard let index = firstIndexMatchingChanges(changes) else { return 0 }
        return self[index]
    }

    @inline(__always)
    func priceChanges(atIndex index: Int) -> [Int]? {
        guard 4 <= index, index < count else { return nil }
...
private extension Int {
    @inline(__always)
    var prices: [Int] {
        (1 ... 2000).reduce(([self % 10], self)) { acc, _ in
            let next = acc.1.nextSecret
            return (acc.0 + [next % 10], next)
        }.0
    }
    
    @inline(__always)
    var nextSecret: Int {
        let step1 = mix(self * 64).prune
        let step2 = step1.mix(step1 / 32).prune
        let step3 = step2.mix(step2 * 2048).prune
        return step3
    }
    
    @inline(__always)
    func mix(_ result: Int) -> Int {
        self ^ result
    }
