...
extension AoC2024 {
    ...
    static func solveDay24Puzzle1(_ mode: AoCMode) -> Int {
        let sections = getPuzzleInput(mode: mode)
        ...
        while !wires.isEmpty {
            wires = wires.filter { wire in
                guard let v1 = values[wire.input1], let v2 = values[wire.input2] else {
                    return true
                }
                values[wire.output] = wire.calculateOutput(v1: v1, v2: v2)
                return false
            }
        }
    ...
    static func solveDay24Puzzle2(_ mode: AoCMode) -> String {
...
private extension [GateWire] {
    var swapped: [GateWire] { filter { $0.isSwappable(with: self) } }
}

private struct GateWire: Equatable {
    let input1: String
    let input2: String
    let operation: String
    let output: String
    
    var isXOR: Bool { operation == "XOR" }
    var isAND: Bool { operation == "AND" }
    var isOR: Bool { operation == "OR" }
    var isOutputZ: Bool { output.hasPrefix("z") }
    
    var hasMixedInputs: Bool {
        (input1.hasPrefix("x") || input1.hasPrefix("y")) &&
        (input2.hasPrefix("x") || input2.hasPrefix("y"))
    }
    
    var hasDoubleZeroSuffix: Bool {
        input1.hasSuffix("00") && input2.hasSuffix("00")
    }
    
    func isSwappable(with wires: [GateWire]) -> Bool {
        if isOutputZ { return output != "z45" && !isXOR }
        
        if hasMixedInputs && !hasDoubleZeroSuffix {
            if isXOR { return !isLinkedToXOR(in: wires) }
            if isAND { return !isLinkedToOR(in: wires) }
        }
        
        return !isOutputZ && isXOR
    }

    func isLinkedToOR(in wires: [GateWire]) -> Bool {
        wires.contains { $0.isOR && ($0.input1 == output || $0.input2 == output) }
    }

    func isLinkedToXOR(in wires: [GateWire]) -> Bool {
        wires.contains { $0.isXOR && ($0.input1 == output || $0.input2 == output) }
    }

    func calculateOutput(v1: Int, v2: Int) -> Int {
        switch operation {
        case "AND": v1 & v2
        case "OR": v1 | v2
        case "XOR": v1 ^ v2
...
private extension [String] {
    func gateValues() -> [String: Int] {
        var result = [String: Int]()
        for input in self {
            let pairs = input.split(separator: ": ")
            result[String(pairs[0])] = Int(pairs[1])!
        }
        return result
    }

    func gateWires() -> [GateWire] {
        let regex = /(\w+)\s+(AND|OR|XOR)\s+(\w+)\s*->\s*(\w+)/
        return compactMap { input in
            guard let match = input.wholeMatch(of: regex) else { return nil }
            return GateWire(
                input1: String(match.output.1),
                input2: String(match.output.3),
                operation: String(match.output.2),
                output: String(match.output.4)
            )
        }
    }
}
