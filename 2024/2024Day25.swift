...
extension AoC2024 {
    ...
    static func solveDay25Puzzle1(_ mode: AoCMode) -> Int {
        let locksAndKeys = getPuzzleInput(mode: mode)
            .components(separatedBy: "\n\n")
            .map(\.as2DArray)
            .locksAndKeys
        
        let locksPins = locksAndKeys.locks.map(\.pinColumns)
        let keysPins = locksAndKeys.keys.map(\.pinColumns)
        
        var overlapCount = locksPins.count * keysPins.count
        for lock in locksPins {
            for key in keysPins where isOverlap(lock, key) {
                overlapCount -= 1
            }
        }
...
private func isOverlap(_ first: [Int], _ second: [Int]) -> Bool {
    zip(first, second).contains { $0 + $1 > 5 }
}

extension [[Character]] {
    /// Checks if the first row is completely locked (all elements are `#`).
    var isLock: Bool {
        guard let firstRow = first else { return false }
        return firstRow.allSatisfy { $0 == "#" }
    }
    
    /// Checks if the last row is completely locked (all elements are `#`).
    var isKey: Bool {
        guard let lastRow = last else { return false }
        return lastRow.allSatisfy { $0 == "#" }
    }
    
    var pinColumns: [Int] {
        guard let columnCount = first?.count else { return [] }
        var result = Array<Int>(repeating: -1, count: columnCount)
        
        for row in self {
            for (columnIndex, character) in row.enumerated() where columnIndex < columnCount {
                if character == "#" {
...
extension [[[Character]]] {
    var locksAndKeys: (locks: Self, keys: Self) {
        var locks = Self()
        var keys = Self()
        ...
        for pattern in self {
            ...
            } else if pattern.isKey {
                keys.append(pattern)
            } else {
                fatalError( "Found invalid pattern \(pattern)")
            }
        }
