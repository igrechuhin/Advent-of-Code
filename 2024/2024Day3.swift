...
extension AoC2024 {
    ...
    static func solveDay3Puzzle2(_ mode: AoCMode) -> Int {
        getPuzzleInput(mode: mode)
            .matches(of: /mul\(\d{1,3},\d{1,3}\)|do\(\)|don't\(\)/)
            .map(\.0)
            .reduce((0, true)) { acc, item in
                switch item {
                case "don't()": (acc.0, false)
                case "do()": (acc.0, true)
                default: (acc.0 + (acc.1 ? mul2Numbers(item) : 0), acc.1)
                }
            }.0
    }
}

private func mul2Numbers(_ str: Substring) -> Int {
    str
        .dropFirst(4) // mul(
        .dropLast(1) // )
        .split(separator: ",")
